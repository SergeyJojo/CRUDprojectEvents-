...
@Composable
fun ItemsListScreen(
    onNavigate: (UiEvent.Navigate) -> Unit, viewModel: ItemsListViewModel = hiltViewModel()
) {
    ...
    Scaffold(snackbarHost = { SnackbarHost(snackbarHostState) }, floatingActionButton = {
        FloatingActionButton(onClick = { viewModel.onEvent(ItemsListEvent.OnAddItemClick) }) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            items(items.value) { item ->
                Items(
                    ...
                    modifier = Modifier
                        .fillMaxSize()
                        .clickable { viewModel.onEvent(ItemsListEvent.OnItemClick(item)) }
                        .padding(16.dp))
            }
        }
    }
